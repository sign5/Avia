...
void GraphGraphicsScene::reloadData() {
    ...
    this->arcItems = std::vector<ArcGraphicsItem*>();
    this->arcItems.clear();
...
//    this->arcItems = std::vector<ArcGraphicsItem*>(arcList.size());
    ...
    for (int i = 0; i < myGraph->getNodeNum(); i++) {
        for (int j = 0; j < myGraph->getNodeNum(); j++) {
            if (myGraph->getFlightData(i, j) != NULL && i != j){
                ArcGraphicsItem *agi = new ArcGraphicsItem(this, nodeItems[i], nodeItems[j]);
                arcItems.push_back(agi);
                this->addItem(agi);
            }
        }
    }
